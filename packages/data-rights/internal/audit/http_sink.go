// http_sink.go — HTTP sink for data-rights audit events.
// 详细注释见 aml-screening/internal/audit/http_sink.go.
...
func (s *HTTPSink) sendOneBatch(batch []Event) error {
	...
	for _, e := range batch {
		w := auditEntryWire{
			...
			Action: e.Action, ResourceType: "dsar_request", ResourceID: e.RequestID,
		}
