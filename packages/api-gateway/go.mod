module github.com/xiongwp/api-gateway

go 1.25.0

// 本仓消费 payment-util/trace（OTel 初始化）+ kms-manage（API key 解密）。
// payment-channel / payment-core / order-core / user-merchant-core 提供 gRPC
// stub 给本仓做 downstream proxy；本仓仅 import 它们的 api/proto/*/v1 包。
// 本地开发要求兄弟仓 clone 到同级目录。CI 替换为 commit / tag 版本。
replace (
