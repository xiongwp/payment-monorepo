...
-- 10 库 × 10 表 = 100 张全局表
-- 占位符：${DB} = 0-9 物理库；${TABLE} = 00-99 全局表序号
-- 路由规则：
--   pi_id 前缀编码分片信息：pi_{dbIdx:1d}{tblIdx:02d}{seq}
--   Charge / Refund 与其 PaymentIntent 同分片
-- ============================================

-- 1. PaymentIntent 主表 payment_intent_${TABLE}
CREATE TABLE IF NOT EXISTS `payment_intent_${TABLE}` (
...
-- 2. Charge 表 charge_${TABLE}（一个 PI 可有多个 Charge：失败 / 重试）
CREATE TABLE IF NOT EXISTS `charge_${TABLE}` (
    `id`                  VARCHAR(64)   NOT NULL,
...
-- 3. PayAction 表 pay_action_${TABLE}（3DS / OTP / PayPassword 等用户挑战条目）
CREATE TABLE IF NOT EXISTS `pay_action_${TABLE}` (
    `id`                VARCHAR(64)  NOT NULL,
    `payment_intent_id` VARCHAR(64)  NOT NULL,
...
-- 4. ExceptionCase 表 exception_case_${TABLE}（差错处理单：过期后迟到回调等）
CREATE TABLE IF NOT EXISTS `exception_case_${TABLE}` (
    `id`                VARCHAR(64)  NOT NULL,
    `payment_intent_id` VARCHAR(64)  NOT NULL,
    `charge_id`         VARCHAR(64)  DEFAULT NULL,
...
-- 5. NotifyLog 表 notify_log_${TABLE}（每次通知客户端的尝试都落一条；cron 扫失败的重发）
CREATE TABLE IF NOT EXISTS `notify_log_${TABLE}` (
    `id`                VARCHAR(64)  NOT NULL,
    `payment_intent_id` VARCHAR(64)  NOT NULL,
    `charge_id`         VARCHAR(64)  DEFAULT NULL,
    `refund_id`         VARCHAR(64)  DEFAULT NULL,
...
-- 5. Refund 表 refund_${TABLE}（一个 Charge 可有多个 Refund：部分退款）
CREATE TABLE IF NOT EXISTS `refund_${TABLE}` (
    `id`                VARCHAR(64)  NOT NULL,
...
-- 7. InboundWebhook 表 inbound_webhook_${TABLE}
...
CREATE TABLE IF NOT EXISTS `inbound_webhook_${TABLE}` (
    `id`                VARCHAR(64)  NOT NULL,
...
CREATE TABLE IF NOT EXISTS `dispute_${TABLE}` (
...
-- 9. DisputeEvent 表 dispute_event_${TABLE}（状态流转日志）
CREATE TABLE IF NOT EXISTS `dispute_event_${TABLE}` (
...
-- 10. AccountingOutbox 表 accounting_outbox_${TABLE}
...
CREATE TABLE IF NOT EXISTS `accounting_outbox_${TABLE}` (
    `id`                 VARCHAR(64)  NOT NULL,
...
-- ─── admin_audit_log_${TABLE}：管理台操作审计（按 actor 路由） ────────────────
