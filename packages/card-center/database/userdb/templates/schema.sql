-- card-center 分片表模板。${DB} = 0..9，${TABLE} = 00..99（globalTblIdx）。
-- generate.sh 把所有 ${DB} / ${TABLE} 替换后产 init/N_init.sql。
--
...
CREATE TABLE IF NOT EXISTS `card_stored_token_${TABLE}` (
...
CREATE TABLE IF NOT EXISTS `card_payment_token_used_${TABLE}` (
    `id`           BIGINT       NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `audit_log_${TABLE}` (
    `id`           BIGINT       NOT NULL AUTO_INCREMENT,
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='card-center audit log shard ${TABLE} (PCI 10.7 7y retention)';
