...
type AccountInstanceManager interface {
	...
	// CreateProvisioned 创建一个 provisioned instance。
...
func (m *accountInstanceManager) PromoteAndDrain(
	...
	// Step 2: 更新 LA.current_active_account_no（跨库）
	// 失败：scheduler 下次 Tick 自动协调
	if err := m.logicalRepo.UpdateCurrentActive(
		ctx, p.LogicalAccountID, p.NewActiveAccountNo, p.NewActivePeriodEnd, p.LogicalAccountVersion,
	); err != nil {
