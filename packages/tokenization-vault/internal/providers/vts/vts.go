...
func New(signingKey []byte) *Provider {
	...
	// 完整 prod 实现: mTLS client + Visa VTS endpoint + apiKey/apiSecret 注入,
	// signingKey 仅 dev/staging 走本地 HMAC.
	if len(signingKey) == 0 {
		env := strings.ToLower(strings.TrimSpace(os.Getenv("APP_ENV")))
		if env == "prod" || env == "production" {
			panic("tokenization-vault VTS: signingKey 不能为空 (APP_ENV=" + env + "). " +
				"prod 必须真接 Visa VTS endpoint + 注入真签名密钥; dev stub HMAC 会让 cryptogram 被卡组织拒.")
		}
