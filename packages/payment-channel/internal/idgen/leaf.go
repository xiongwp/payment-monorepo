// Package idgen 号段式 ID 生成器。与 order-core 的 idgen 同构，只是 biz_tag
// 换成 payment-channel 自己的三个：acquirer_tx / webhook_raw / channel_token。
package idgen
...
const (
	BizTagAcquirerTx   = "paychan.acquirer_tx"
	BizTagWebhookRaw   = "paychan.webhook_raw"
	BizTagChannelToken = "paychan.channel_token"
)
...
func New(metaDB *gorm.DB, logger *zap.Logger) (IDGenerator, error) {
	...
	for _, biz := range []struct{ name, desc string }{
		{BizTagAcquirerTx, "AcquirerTx id"},
		{BizTagWebhookRaw, "WebhookRaw id"},
		{BizTagChannelToken, "ChannelToken id"},
	} {
