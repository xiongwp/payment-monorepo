#!/usr/bin/env bash
# ─── monorepo 一键起全栈：按依赖顺序拉起 14 服务 ─────────────────────────
#
# 依赖图（顶向下）：
#   1. payment-stack 共享网络（外部 docker network）
#   2. config-center        （所有业务服务的强依赖；启动期同步拉 snapshot）
#   3. id-generator / kms-manage / etcd / kafka 基础设施
#   4. user-merchant-core / order-core / payment-core / accounting-system /
#      payment-channel / risk-manage / clearing-settlement / reconplatform /
#      card-center / card-payment
#   5. api-gateway / *-admin-web 边缘
#
# 用法：
#   bash deploy-stack.sh up      # 起全栈（按依赖顺序）
#   bash deploy-stack.sh down    # 停全栈
#   bash deploy-stack.sh status  # 各 stack docker compose ps
#   bash deploy-stack.sh seed    # 初始化 config-center 种子配置
#
# 注意：每个 stack 自己 build 镜像；确保 ../payment-util 等兄弟仓在 packages/ 下，
# 否则 Dockerfile 走 git clone（需要 GITHUB_TOKEN secret）。
set -euo pipefail
